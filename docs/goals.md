...
# 이번 주 목표 
- 첫 번째 주는 **`개발 환경과 프로그래밍 언어에 익숙`**, 그렇다면 이번 주는?
- 시작 -> 연습(활용), 이번 주는 **`프로그래밍 언어/라이브러리가 제공하는 다양한 기능들을 활용`** 하는 것이 목표
- 이를 위해서는...
  - 구현 때 의식한다. **`Java 라이브러리가 제공하는 기능이지 않을까?`**
  - 테스트 때 의식한다. **`JUnit, AssertJ 라이브러리가 제공하는 테스트 방법이지 않을까?`**
  - 내부를 들여다본다. **`디버깅을 통해 라이브러리 내부를 들여다 보자`**
...
- 수정된 둘째 주 목표: `행동`을 기준으로 만든 단위 테스트에 맞는 리팩토링 기준점 만들기
  - **`행동`**: 객체의 **`상태`** 에 영향을 주는 메서드, 상태가 변하는 이유는 외부에서 요청이 들어와 요청에 맞게 행동하기 때문
  - 즉, 행동은 public 메서드
...
- 자주 사용되는 테스트 모범 패턴을 학습한다: [precourse-log 리포지토리](https://github.com/ykmxxi/precourse-log)에 기록
...
- 여러 역할을 수행하는 큰 함수를 단일 역할을 수행하는 작은 함수로 분리한다
  - 여러 역할이란 무엇일까?
  - 언제 분리할까? `private` 메서드가 직접 상태를 변경한다면 `public`이 아닐까?
  - 어디까지 분리해야할까? 일단 최대한 작게 분리한다
...
- 1주 차 공통 피드백 반영
  ...
  - 이번 미션 때 테스트 실행을 디버깅으로만 진행하니 좋은점: 굳이 통합 테스트(컨트롤러 단)를 할 필요가 안느껴짐, 실제로 이상한 과정을 통해 정상 결과가 나온 부분을 바로 잡음
    - 왜 `Mockito` 라이브러리를 mission-utils 에서 전파시키지 않았는지 이해가 됨
    - 단위 테스트를 통해 도메인, 서비스의 흐름을 검증
    - 위 계층의 클라이언트의 흐름을 알 수 있었음 -> 굳이 통합 테스트를 할 필요가 안느껴짐
  - 당연시 여기던 것, 당연히 안다고 생각한 것에 계속 물음을 던지자
    - `$./gradlew clean test`를 입력하면 어떤 과정을 거쳐 테스트 실행이 일어날까?
- 회고
  - 좋은 회고 방법은? 내가 회고로 얻고 싶은 것은 무엇일까?
    - 느낀 감정들, 문제가 된 상황을 그때 그때 기록 
    - 오랜 구현 시간이 걸리는 이유를 확실히 알게됐음: 너무 작은 부분 or 큰 단위 or 현재 불필요한(성능 개선 등) 부분에 집착 -> 찾아보는데 시간이 걸림 -> 결정하지 못함  
  - 지원서에 작성한 목표를 달성하고 있나? 그 이유는?
    - 목표도 리팩토링, 어느정도 틀을 가지고 시작해도 실제 진행하면서 생각이 달라질 수 있음
